...
export default function Register() {
    ...
    // Initialize directly from localStorage — no useEffect needed, avoids cascading render warning
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            // Fixed: removed emoji from string since JSX already renders one
            setSuccess("Account created successfully! Redirecting to login...");

            // Wait 2 seconds so they can see the success badge, then push to Login
            setTimeout(() => {
                navigate('/login');
            }, 2000);
        } catch (err) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-700 bg-slate-900">
            ...
            {/* Cross-fading Smooth Mesh Background Layers */}
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className={`rounded-2xl border shadow-2xl p-8 relative transition-all duration-700 ${darkMode ? 'bg-slate-900/85 border-slate-800 shadow-black/50' : 'bg-white/95 border-slate-200/80 shadow-slate-200/60'}`}>
                    ...
                    {/* Success Banner — fixed: emoji only in JSX, not duplicated in state string */}
                    {success && (
